...
def plot_result(epoch, num, figure_dir, preds, X, Y, mode="valid"):
    ...
    for i in range(min(num, len(X))):
        ...
        plt.subplot(4, 2, 7)
        plt.plot(
            t, ideal_denoised_signal,
            label='Ideal denoised signal',
            linewidth=0.1,
        )
        ...
        plt.savefig(
            os.path.join(
                figure_dir,
                "epoch{:03d}_{:03d}_{:}.png".format(epoch, i, mode)
            ),
            bbox_inches='tight'
        )
        # plt.savefig(
        #   os.path.join(
        #       figure_dir, "epoch%03d_%03d.pdf" % (epoch, i)
        #   ),
        #   bbox_inches='tight'
        # )
        plt.close(i)
...
def plot_result_thread(i, epoch, preds, X, Y, figure_dir, mode="valid"):
    ...
    plt.plot(
        t, ideal_denoised_signal,
        'k',
        label='Ideal denoised signal', linewidth=0.5,
    )
    plt.ylim(signal_ylim)
    plt.xlabel("Time (s)")
    plt.legend(loc='lower left')
    ...
    plt.plot(
        t, denoised_signal,
        'k',
        label='Denoised signal', linewidth=0.5,
    )
    plt.ylim(signal_ylim)
    plt.xlabel("Time (s)")
    plt.legend(loc='lower left')
...
def postprocessing_test(
    i, preds, X, fname,
    figure_dir=None, result_dir=None,
    signal_FT=None, noise_FT=None,
    data_dir=None,
):
    ...
    if result_dir is not None:
        try:
            ...
            os.makedirs(
                os.path.dirname(os.path.join(result_dir, fname[i].decode())),
                exist_ok=True,
            )
            np.savez(
                os.path.join(result_dir, fname[i].decode()),
    ...
    if figure_dir is not None:
        ...
        if data_dir is not None:
            raw_data = np.load(
                os.path.join(data_dir, fname[i].decode().split('/')[-1])
            )
            itp = raw_data['itp']
            its = raw_data['its']
            ix1 = (750 - 50) / 100
        ...
        try:
            plt.savefig(
                os.path.join(
                    figure_dir,
                    fname[i].decode().rstrip('.npz') + '_FT.png'
                ),
                bbox_inches='tight',
            )
            # plt.savefig(
            #   os.path.join(
            #     figure_dir,
            #     fname[i].decode().rstrip('.npz')+'_FT.pdf'
            #   ),
            # bbox_inches='tight',
            # )
        except FileNotFoundError:
        ...
        plt.plot(
            t1, denoised_signal,
            'k',
            linewidth=0.5, label='Recovered signal',
        )
        plt.legend(loc='lower left', fontsize='medium')
        plt.xlim([np.around(t1[0]), np.around(t1[-1])])
        plt.ylim(signal_ylim)
        ...
        plt.plot(
            t1, denoised_noise,
            'k',
            linewidth=0.5, label='Recovered noise',
        )
        plt.legend(loc='lower left', fontsize='medium')
        plt.xlim([np.around(t1[0]), np.around(t1[-1])])
        plt.xlabel("Time (s)", fontsize='large')
        ...
        plt.savefig(
            os.path.join(
                figure_dir,
                fname[i].decode().rstrip('.npz') + '_wave.png'
            ),
            bbox_inches='tight'
        )
        # plt.savefig(
        #   os.path.join(
        #       figure_dir,
        #       fname[i].decode().rstrip('.npz')+'_wave.pdf'
        #   ),
        #   bbox_inches='tight'
        # )
        plt.close(i)
...
def postprocessing_pred(i, preds, X, fname, figure_dir=None, result_dir=None):
    ...
    if figure_dir is not None:
        ...
        t_FT = np.linspace(
            config.time_range[0], config.time_range[1],
            X.shape[2],
        )
        f_FT = np.linspace(
        ...
        try:
            ...
            #   os.path.join(
            #       figure_dir,
            #       fname[i].split('/')[-1].rstrip('.npz')+'_FT.pdf'
            #   ),
            #   bbox_inches='tight'
            # )
        except FileNotFoundError:
...
def save_results(
        mask, X, fname, t0,
        save_signal=True, save_noise=True,
        result_dir="results",
        ):
    ...
    if save_signal:
        ...
        denoised_signal = np.transpose(denoised_signal, [0, 3, 2, 1])
        # nbt, nt, nst, nch
    if save_noise:
...
def plot_figures(mask, X, fname, figure_dir="figures"):
    ...
    try:
        ...
        os.makedirs(
            os.path.dirname(
                os.path.join(figure_dir, fname.rstrip('.npz') + '_FT.png'),
            ),
            exist_ok=True,
        )
        plt.savefig(
