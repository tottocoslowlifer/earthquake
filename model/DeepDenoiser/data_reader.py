...
def normalize(data, window=200):
    ...
    mean_interp = interp1d(t, mean, kind="slinear")(t_interp)
    data_numerator = data - mean_interp[np.newaxis, :, np.newaxis]
    data_denominator = std_interp[np.newaxis, :, np.newaxis]
...
def normalize_batch(data, window=200):
    ...
    for i in range(std.shape[1]):
        std[:, i] = np.std(
            data_pad[:, :, i * shift: i * shift + window, :],
            axis=(1, 2, 3),
        )
        mean[:, i] = np.mean(
            data_pad[:, :, i * shift: i * shift + window, :],
...
def dataset_map(
        ...
        output_shapes=None, num_parallel_calls=None,
        name=None
        ):
...
class DataReader(object):
    ...
    def add_queue(self):
        with tf.device('/cpu:0'):
            self.sample_placeholder = tf.compat.v1.placeholder(
                dtype=tf.float32,
                shape=None
            )
            self.target_placeholder = tf.compat.v1.placeholder(
                dtype=tf.float32,
                shape=None,
            )
    ...
    def add_event(self, sample, channels, j):
        while np.random.uniform(0, 1) < 0.2:
            ...
            if channels not in self.buffer_channels_signal:
                self.buffer_channels_signal[channels] = self.signal[
                    self.signal['channels'] == channels
                ]
            fname = os.path.join(
                self.signal_dir,
                self.buffer_channels_signal[channels].sample(n=1).iloc[0][
                    'fname'
                ]
            )
            try:
            ...
            if meta_signal['snr'][j] > self.config.snr_threshold:
                ...
                tmp_signal[:, -shift:] = meta_signal['data_FT'][
                    :, self.X_shape[1]: 2 * self.X_shape[1] + shift, j
                ]
                if (
                    np.isinf(tmp_signal).any()
                    or np.isnan(tmp_signal).any()
                    or (not np.any(tmp_signal))
                ):
                    continue
    ...
    def thread_main(self, sess, n_threads=1, start=0):
        ...
        while not stop:
            ...
            for i in index:
                fname_signal = os.path.join(
                    self.signal_dir,
                    self.signal.iloc[i]['fname']
                )
                try:
                    ...
                    logging.error(
                        "Failed reading signal: {}".format(fname_signal)
                    )
                    continue
                ...
                if channels not in self.buffer_channels_noise:
                    self.buffer_channels_noise[channels] = self.noise[
                        self.noise['channels'] == channels
                    ]
                fname_noise = os.path.join(
                    self.noise_dir,
                    (
                        self.buffer_channels_noise[channels]
                        .sample(n=1)
                        .iloc[0]['fname']
                    ),
                )
                try:
                    ...
                    logging.error(
                        "Failed reading noise: {}".format(fname_noise)
                    )
                    continue
                ...
                if (
                    np.isinf(tmp_noise).any()
                    or np.isnan(tmp_noise).any()
                    or (not np.any(tmp_noise))
                ):
                    continue
                ...
                while ratio <= 0:
                    ratio = (
                        ...
                        + np.random.randn() * self.config.noise_std
                    )
                # ratio = np.random.uniform(
                #           self.config.noise_low,
                #           self.config.noise_high,
                # )
                tmp_noisy_signal = tmp_signal + ratio * tmp_noise
                noisy_signal = np.stack(
                    [tmp_noisy_signal.real, tmp_noisy_signal.imag],
                    axis=-1,
                )
                if (
                    np.isnan(noisy_signal).any()
                    or np.isinf(noisy_signal).any()
                ):
                    continue
                noisy_signal = noisy_signal / np.std(noisy_signal)
                tmp_mask_numerator = (
                    np.abs(tmp_signal)
                    + np.abs(ratio * tmp_noise)
                    + 1e-4
                )
                tmp_mask = np.abs(tmp_signal) / tmp_mask_numerator
...
class DataReader_test(DataReader):
    ...
    def add_queue(self):
        ...
        self.ratio_placeholder = tf.compat.v1.placeholder(
            dtype=tf.float32, shape=None
        )
        self.signal_placeholder = tf.compat.v1.placeholder(
            dtype=tf.complex64, shape=None
        )
        self.noise_placeholder = tf.compat.v1.placeholder(
            dtype=tf.complex64, shape=None
        )
        self.fname_placeholder = tf.compat.v1.placeholder(
            dtype=tf.string, shape=None
        )
        self.queue = tf.queue.PaddingFIFOQueue(
            self.queue_size,
            ['float32', 'float32', 'float32',
             'complex64', 'complex64', 'string'],
            shapes=[
                self.config.X_shape,
                self.config.Y_shape,
                [],
    ...
    def thread_main(self, sess, n_threads=1, start=0):
        ...
        for i in index:
            ...
            tmp_mask_numerator = (np.abs(tmp_signal)
                                  + np.abs(ratio * tmp_noise)
                                  + 1e-4
                                  )
            tmp_mask = np.abs(tmp_signal) / tmp_mask_numerator
        ...
        for i in index:
            ...
            f, t, tmp_signal = scipy.signal.stft(
                scipy.signal.detrend(
                    np.squeeze(
                        data_signal['data'][shift: self.config.nt + shift]
                    )
                ),
                fs=self.config.fs,
                nperseg=self.config.nperseg,
            ...
            noisy_signal = np.stack(
                [tmp_signal.real, tmp_signal.imag],
                axis=-1,
            )
            if (
                np.isnan(noisy_signal).any()
                or np.isinf(noisy_signal).any()
                or (not np.any(noisy_signal))
            ):
                continue
            std_noisy_signal = np.std(noisy_signal)
...
class DataReader_pred:
    ...
    def get_data_shape(self):
        ...
        # data = np.load(
        #           os.path.join(self.signal_dir, fname),
        #           allow_pickle=True
        #        )["data"]
        ...
        if self.sampling_rate != 100:
            ...
            t_interp = np.linspace(
                0, 1,
                np.int(np.around(data.shape[-1] * 100.0 / self.sampling_rate))
            )
        ...
        logging.info(
            f"Input data shape:{tmp_signal.shape} measured on file {base_name}"
        )
    ...
    def read_mseed(self, fname):
        ...
        mseed = mseed.detrend(
            "spline",
            order=2,
            dspline=5 * mseed[0].stats.sampling_rate
        )
        ...
        if mseed[0].stats.sampling_rate != self.sampling_rate:
            logging.warning(
                (
                    f"Sampling rate {mseed[0].stats.sampling_rate} "
    ...
    def __getitem__(self, i):
        ...
        if self.sampling_rate != 100:
            ...
            data = interp1d(t, data, kind="slinear")(t_interp)
        # sos = scipy.signal.butter(4, 0.1, 'high', fs=100, output='sos')
        # for stability of long sequence
        ...
        # return (
        #           noisy_signal.astype(self.dtype),
        #           std_noisy_signal.astype(self.dtype),
        #           fname
        #         )
