...
def crop_and_concat(net1, net2):
    ...
    # offsets = [
    #   0,
    #   (net2_shape[1] - net1_shape[1]) // 2,
    #   (net2_shape[2] - net1_shape[2]) // 2,
    #   0,
    # ]
    ...
    # #     offsets = [
    #           0,
    #           (net1_shape[1] - net2_shape[1]) // 2,
    #           (net1_shape[2] - net2_shape[2]) // 2,
    #           0,
    #       ]
...
class UNet:
    ...
    def add_placeholders(self, input_batch=None, mode='train'):
        ...
        self.drop_rate = tf.compat.v1.placeholder(
            dtype=tf.float32,
            name="drop_rate",
        )
    ...
    def add_loss_op(self):
        if self.loss_type == "cross_entropy":
            with tf.compat.v1.variable_scope("cross_entropy"):
                ...
                if (np.array(self.class_weights) != 1).any():
                    ...
                    # loss_map = tf.nn.sigmoid_cross_entropy_with_logits(
                    #               logits=flat_logits,
                    #               labels=flat_labels,
                    # )
            ...
            with tf.compat.v1.variable_scope("IOU"):
                ...
                for i in range(1, self.n_class):
                    intersection = eps + tf.reduce_sum(
                        input_tensor=(
                            self.preds[:, :, :, i] * self.Y[:, :, :, i]
                        ),
                        axis=[1, 2],
                    )
                    union = (
                        eps
                        + tf.reduce_sum(input_tensor=self.preds[:, :, :, i],
                                        axis=[1, 2])
                        + tf.reduce_sum(input_tensor=self.Y[:, :, :, i],
                                        axis=[1, 2])
                    )
                    loss += 1 - tf.reduce_mean(
                        input_tensor=intersection / union
                    )
        ...
        if self.weight_decay > 0:
            with tf.compat.v1.name_scope('weight_loss'):
                tmp = tf.compat.v1.get_collection(
    ...
    def add_training_op(self):
        ...
        tmp = tf.compat.v1.summary.scalar(
            "learning_rate",
    ...
    def train_on_batch(
        ...
        _, step_summary, step, loss = sess.run(
            [self.train_op, self.summary_train, self.global_step, self.loss],
            feed_dict=feed,
        )
    ...
    def valid_on_batch(
        ...
        step_summary, step, loss, preds = sess.run(
            [self.summary_valid, self.global_step, self.loss, self.preds],
            feed_dict=feed,
        )
        summary_writer.add_summary(step_summary, step)
        return loss, preds
